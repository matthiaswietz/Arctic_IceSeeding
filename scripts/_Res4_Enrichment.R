...
## Prepare ASVs
# Prevalence filter: >=1 count in >=10 samples
asv <- ASV.bac %>% 
  filter(rowMeans(across(everything(), ~ . >= 1)) >= 0.1) %>%
...
# Extract results
# Join niche of most abundant ASV per Genus
FracBacRes = FracBac$res %>%
  ...
  mutate(enrichFrac = case_when(
    ...
    TRUE ~ NA_real_)) %>%
  right_join(nichePref.bac, by=c("taxon"="Genus")) %>%
...
FracEukRes = FracEuk$res %>%
  ...
  mutate(enrichFrac = case_when(
    ...
    TRUE ~ NA_real_)) %>%
  drop_na() %>%
  right_join(nichePref.euk, by=c("taxon"="Genus")) %>%
  mutate(niche = case_when(
    is.na(niche) ~"unclear", T~niche),
    tax="Eukaryotes") %>%
  drop_na(fraction) 
...
# Omit "double taxa" (e.g. keep Pelagomonas; remove UCs)
# Export size 6x8
rbind(FracEukRes, FracBacRes) %>%
  filter(!taxon %in% c(
    ...
    #"Ventricleftida uc", "Raphid-pennate uc",
    "Prymnesiophyceae_Clade_B3 uc","Pyramimonadales uc",
    "Stramenopiles ucX","Prorocentrum","Ochrophyta uc",
    "Dolichomastigaceae-B","NPK2-lineage uc",
    "Suessiaceae uc","Pelagomonadales_clade_B1",
    "Pelagomonadales_clade_B2","Pelagomonadales_clade_A uc")) %>%
